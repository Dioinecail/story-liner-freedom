...
namespace Self.ArticyImporter
{
    public static class ArticyProjectImporter
    {
        ...
        private static Dictionary<HexValue, Character> CreateCharactersFromImport(ArticyData data, string path) {
            ...
            HashSet<HexValue> characterIds = entities.Children
                .Where(node => !node.Type.Equals("EntitiesUserFolder"))
                .Select(node => {
                    return node.Id;
                }).ToHashSet();

            foreach (var n in entities.Children
                .Where(node => node.Type.Equals("EntitiesUserFolder"))) {
                foreach (var child in n.Children) {
                    characterIds.Add(child.Id);
                }
            }
            ...
            foreach (var character in characters) {
                if (character.Type.Equals("UserFolder"))
                    continue;
